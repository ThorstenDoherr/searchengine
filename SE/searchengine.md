...
You should only specify the base table once per SearchEngine installation. The search tables will always be searched within the same base table. Depending on the setup, this may be a singular search project between two tables or a extensive framework catered for a multitude of search projects on a central base table. You should also assign a dedicated result table for every search project aka. search table. Hint: when you leave the result table field empty, a table name will be automatically created based on the search table name plus "_result.dbf".   
[[importbase]](#importbase)  
[[importsearch]](#importsearch)  
[[result]](#result)  
